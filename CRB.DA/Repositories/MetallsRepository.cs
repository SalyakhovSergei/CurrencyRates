    ...
    public class MetallsRepository: IMetallRepository
    {
        ...
        public async Task<int> CreateAsync(MetallDTO item, IDbTransaction transaction = null)
        {
            try
            {
                ...
                {
                    var id = await conn.InsertSingleAsync(item, r => r.Id, transaction, tableName: "public.metall");
                    return id;
                }
        ...
        {
            using var conn = _connFactory.GetConnection(transaction);
            var sql = $@"SELECT * FROM public.metall WHERE ""AlphaCode""=@code and ""Date""='{date.Value:dd/MM/yyyy}' LIMIT 1";
        ...
        {
            using var connection = _connFactory.GetConnection(transaction);
            var sql = $@"SELECT ""Date"" FROM public.metall ORDER BY ""Date"" DESC LIMIT 1";
