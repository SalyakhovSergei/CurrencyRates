    ...
    public class CurrencyRepository : ICurrencyRepository
    {
        ...
        public async Task<int> CreateAsync(CurrencyDTO item, IDbTransaction transaction = null)
        {
            try
            {
                ...
                if (CheckItemInDb(item)?.Date.LocalDateTime != item.Date.LocalDateTime)
                {
                    var id = await connection.InsertSingleAsync(item, r => r.Id, transaction, tableName: "public.currency");
                    return id;
                }
        ...
        public Task<int> UpdateAsync(CurrencyDTO item, IDbTransaction transaction = null)
        {
            try
            {
                ...
                var sql = $@"UPDATE public.currency SET ""Price""={item.Price.ToString().Replace(',', '.')}, ""Scale""={item.Scale} 
        ...
        {
            using var conn = _connFactory.GetConnection(transaction);
            var sql = $@"SELECT * FROM public.currency WHERE ""AlphaCode""=@code and ""Date""='{date.Value:MM/dd/yyyy}' LIMIT 1";
        ...
        {
            using var conn = _connFactory.GetConnection(transaction);
            var sql = $@"SELECT * FROM public.currency WHERE ""NumericCode""=@code and ""Date""='{date.Value:MM/dd/yyyy}' LIMIT 1";
            var result = conn.QuerySingleOrDefault<CurrencyDTO>(sql, new {code}, transaction);
            return result;
        }
        ...
        {
            using var connection = _connFactory.GetConnection(transaction);
            var sql = $@"SELECT ""Date"" FROM public.currency ORDER BY ""Date"" DESC LIMIT 1";
