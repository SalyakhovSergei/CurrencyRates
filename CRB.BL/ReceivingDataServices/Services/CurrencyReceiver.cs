...
namespace CRB.BL.ReceivingDataServices.Services
{
    public class CurrencyReceiver: ICurrencyReceiver
    {
        ...
        public async Task<string> ReceiveCurrency()
        {
            string result = "Currency refreshed";
            RatesOnDateResponse response = new RatesOnDateResponse();
      
            try
            {
                response = _axiomaOmnIcurrencyrates.DailyFxratesAsync(DateTimeOffset.Now.Date).Result;
                var currencyRate = _mapper.Map<CurrencyRate>(response);
                var rates = currencyRate.Rates;

                foreach (var rate in rates)
                {
                    rate.Date = currencyRate.Date.ToUniversalTime();
