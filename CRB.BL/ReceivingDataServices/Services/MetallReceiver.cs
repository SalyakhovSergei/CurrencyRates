...
namespace CRB.BL.ReceivingDataServices.Services
{
    public class MetallReceiver: IMetallReciever
    {
        ...
        public async Task<string> ReceiveMetalls()
        {
            string result = "Metals refreshed";
            MetalsOnDateResponse response = new MetalsOnDateResponse();

            try
            {
               response = _axiomaClient.MetalsAsync(DateTimeOffset.Now.Date).Result;
               ...
               {
                   rate.Date = metallRate.Date.ToUniversalTime();
